...
def validate_command_security(command: str) -> Tuple[bool, str]:
    ...
    for exe_name, segment in executables:
        ...
        if exe_name in BLOCKED_EXECUTABLES:
            ...
            return (
                False,
                f"Blocked because executable '{exe_name}' is in the deny list for safety reasons",
            )
        ...
        if exe_name not in ALLOWED_EXECUTABLES:
            ...
            if shutil.which(exe_name):
                ...
                return (
                    False,
                    f"Blocked because executable '{exe_name}' is not in the allowlist. Add it to ALLOWED_EXECUTABLES if safe",
                )
            else:
                logger.warning(f"Executable not found: {exe_name}")
                return (
                    False,
                    f"Blocked because executable '{exe_name}' is not recognized or not in the allowlist",
                )
...
def execute_command_safely(
    command: str, timeout: int = EXECUTION_TIMEOUT
) -> Dict[str, Any]:
    """
...
def supervisor_node(state: AgentState) -> Dict[str, Any]:
    ...
    if len(state["history"]) > 0:
        ...
        if last_ai_msg.get(
            "role"
        ) == "model" and "⚠️ **Approval Required**" in last_ai_msg.get("content", ""):
            ...
            if user_msg in ["yes", "y", "approve", "do it", "execute", "go ahead"]:
                ...
                return {
                    ...
                    "final_response": "Execution cancelled. Let me know if you need anything else!",
                }
    ...
    if intent == "MUTATION" and command:
        print(
            f"⚠️ [Supervisor]: Mutation detected for command: {command}. Routing to Approval Gate."
        )
        return {
...
def docker_agent_node(state: AgentState) -> Dict[str, Any]:
    ...
    cmd = (
        state.get("proposed_command")
        or "docker ps --format 'table {{.ID}}\t{{.Names}}\t{{.Status}}'"
    )
    raw_output = subprocess.run(cmd, shell=True, capture_output=True, text=True).stdout
    return {
        "raw_logs": f"Docker Environment Diagnostics:\n{raw_output}",
        "next_step": "SYNTHESIS",
    }
...
def k8s_agent_node(state: AgentState) -> Dict[str, Any]:
    ...
    cmd = (
        state.get("proposed_command") or "kubectl get pods,services,deployments -o wide"
    )
    raw_output = subprocess.run(cmd, shell=True, capture_output=True, text=True).stdout
    return {
        "raw_logs": f"Kubernetes Cluster State:\n{raw_output}",
        "next_step": "SYNTHESIS",
    }
...
def git_agent_node(state: AgentState) -> Dict[str, Any]:
    ...
    raw_output = (
        ...
        or "No active local git configuration found."
    )
    return {
        "raw_logs": f"Git Revision Timeline:\n{raw_output}",
        "next_step": "SYNTHESIS",
    }
...
def terraform_agent_node(state: AgentState) -> Dict[str, Any]:
    ...
    raw_output = (
        subprocess.run(cmd, shell=True, capture_output=True, text=True).stdout
        or "No local terraform plan state found."
    )
    return {
        "raw_logs": f"Terraform Workspace Plan State:\n{raw_output}",
        "next_step": "SYNTHESIS",
    }
...
def direct_answer_node(state: AgentState) -> Dict[str, Any]:
    ...
    response = client.models.generate_content(
        model="gemini-2.5-flash", contents=state["message"]
    )
    return {"final_response": response.text}
